...
// ── K-factor ─────────────────────────────────────────────────────────────
//
// A single fixed K applied to every game of a tournament gets both ends
// of the tournament wrong:
//
//   * Early on, every rating is still close to its (arbitrary) starting
//     value and carries essentially zero information. A fixed, moderate
//     K (the classic 32) reacts too *slowly* to that — the rating pool
//     is overdamped right when it most needs to move fast to find its
//     footing.
//   * Late in a long tournament, ratings have mostly converged and each
//     additional game is mostly telling you about game-to-game noise
//     (time pressure variance, a drawish opening, etc.) rather than true
//     strength. The same fixed K keeps applying full-size corrections,
//     so the pool becomes underdamped / jumpy exactly when it should be
//     settling down.
//
// The fix used here is the standard statistical one: the uncertainty in
// a rating estimated from `n` game outcomes shrinks like `1/sqrt(n)` (a
// running proportion estimate), so we shrink the *step size* of each
// update the same way. See `dynamic_k` below.
//
// ── PRNG ─────────────────────────────────────────────────────────────────
//
// The pairing scheduler (`PairingPool`) needs an RNG that's reproducible
// from a seed (so a tournament's draw sequence can be replayed/debugged).
// That's the ONLY property it needs, and `rand::rngs::StdRng` seeded via
// `SeedableRng::seed_from_u64` already provides exactly that — same
// approach `zobrist.rs` uses elsewhere in this crate — so there is no
// hand-rolled generator here.
...
impl Termination {
    ...
    pub fn label(self) -> &'static str {
        ...
        match self {
            ...
            MutualElimination => "mutual royal elimination",
            PlyLimit => "ply limit (adj.)",
        }
    }
    pub const ALL: [Termination; 10] = {
        use Termination::*;
        [
            Checkmate,
            Extinction,
...
// Dynamic K-factor
// ─────────────────────────────────────────────────────────────────────────
/// Statistically-motivated K-factor decay, shared by the main tournament
/// ELO tracker and the parameter-evolution population ratings.
///
/// `k_initial`  — K for a participant with zero recorded games. Should be
///                 noticeably higher than a "classic" fixed K (32) so a
///                 brand-new rating snaps toward its true strength fast.
/// `k_floor`    — the asymptotic minimum K, approached as games_played
///                 grows without bound. Should be noticeably lower than
///                 32 so a long-running, converged rating stops reacting
///                 to single-game noise.
/// `k_scale`    — games-played scale over which the excess above the
///                 floor roughly halves (at `games_played == k_scale`,
///                 the `1/sqrt(1+n/scale)` term is `1/sqrt(2) ≈ 0.71`).
///
/// The `1/sqrt(n)` shape is not arbitrary: it's the same shape as the
/// standard error of a proportion estimated from `n` Bernoulli trials,
/// which is exactly what a win/draw/loss rating is. Using it for the
/// update step size means the rating *behaves* like a running estimate
/// whose confidence grows with sample size, rather than a fixed-gain
/// control loop that's either too sluggish or too twitchy depending on
/// how far along the tournament is.
pub fn dynamic_k(games_played: u32, k_initial: f64, k_floor: f64, k_scale: f64) -> f64 {
    let n = games_played as f64;
    let excess = (k_initial - k_floor).max(0.0);
    let scale = k_scale.max(0.01);
    k_floor + excess / (1.0 + n / scale).sqrt()
}
// ─────────────────────────────────────────────────────────────────────────
// ELO tracking
// ─────────────────────────────────────────────────────────────────────────
...
pub struct EloTracker {
    initial_rating: f64,
    /// K-factor for a participant that has played zero games yet.
    k_initial: f64,
    /// Asymptotic floor K approaches as a participant's game count grows.
    k_floor: f64,
    /// Games-played scale over which the K decay happens (see `dynamic_k`).
    k_scale: f64,
    ratings: HashMap<EngineType, f64>,
    /// Per-engine game count, used to look up the *current* K-factor for
    /// that engine. Updates are intentionally asymmetric: each side's
    /// rating moves by its OWN K times the score surprise, not a shared
    /// K — an engine with a thousand games and one with three games
    /// playing each other should not move by the same amount.
    games_count: HashMap<EngineType, u32>,
...
impl EloTracker {
    pub fn new() -> Self {
        Self {
            ...
            k_scale: 12.0,
            ratings: HashMap::new(),
            games_count: HashMap::new(),
            history: Vec::new(),
            matchups: HashMap::new(),
            game_log: Vec::new(),
        }
    }
    /// Override the K-decay shape. Not wired into the GUI (the defaults
    /// are deliberately chosen to be broadly sane), but available for
    /// terminal / scripted tuning.
    pub fn set_k_params(&mut self, k_initial: f64, k_floor: f64, k_scale: f64) {
        self.k_initial = k_initial.max(0.0);
        self.k_floor = k_floor.max(0.0).min(self.k_initial);
        self.k_scale = k_scale.max(0.01);
    }
    pub fn rating(&self, engine: &EngineType) -> f64 {
        *self.ratings.get(engine).unwrap_or(&self.initial_rating)
    }
    pub fn games_played_by(&self, engine: &EngineType) -> u32 {
        self.games_count.get(engine).copied().unwrap_or(0)
    }
    /// The K-factor that would currently apply to `engine`'s next game.
    pub fn effective_k(&self, engine: &EngineType) -> f64 {
        dynamic_k(
            self.games_played_by(engine),
                  self.k_initial,
                  self.k_floor,
                  self.k_scale,
        )
    ...
    pub fn record(
        ...
        // Each side uses its OWN dynamic K, based on its OWN game count
        // so far. This is intentionally asymmetric — see the struct doc.
        let k_white = self.effective_k(&white);
        let k_black = self.effective_k(&black);
        let white_delta = k_white * (actual_white - expected_white);
        // actual_black - expected_black == expected_white - actual_white
        let black_delta = k_black * (expected_white - actual_white);
        ...
        *self.games_count.entry(white.clone()).or_insert(0) += 1;
        *self.games_count.entry(black.clone()).or_insert(0) += 1;
    ...
    pub fn print_results_matrix(&self) {
        ...
        if engines.is_empty() {
            clog!("No tournament data to display.");
            return;
        }
        engines.sort_by(|a, b| {
            self.rating(b)
        ...
        for (i, e) in engines.iter().enumerate() {
            clog!("  {}. {} — {:.0}", i + 1, e.name(), self.rating(e));
        }
        let name_width = 20;
        let cell_width = 9;
        clog!("\n═══ Head-to-Head Results (W/L/D from row's perspective) ═══");
        let mut header = format!("{:>width$} │", "", width = name_width);
        for e in &engines {
            let short = short_name(e.name(), cell_width);
            header.push_str(&format!("{:^width$}│", short, width = cell_width));
        }
        clog!("{}", header);
        let mut sep = format!("{:─>width$}─┼", "", width = name_width);
        for _ in &engines {
            sep.push_str(&format!("{:─>width$}┼", "", width = cell_width));
        }
        clog!("{}", sep);
        for row in &engines {
            let rn = short_name(row.name(), name_width);
            let mut line = format!("{:>width$} │", rn, width = name_width);
            for col in &engines {
                if row == col {
                    line.push_str(&format!("{:^width$}│", "---", width = cell_width));
                } else {
    ...
    pub fn print_detailed_report(&self) {
        if self.game_log.is_empty() {
            clog!("No tournament data to report.");
            return;
        }
        self.print_global_section();
        self.print_per_engine_section();
        self.print_colour_split_matrix();
        clog!("(Use `tstats <engineA> <engineB>` for per‑pairing detail.)\n");
    }
    fn print_global_section(&self) {
        ...
        clog!(
            "  K-factor: {:.1} → {:.1} (half-life ≈ {:.0} games)",
              self.k_initial, self.k_floor, self.k_scale
        );
    ...
    fn print_colour_split_matrix(&self) {
        ...
        clog!("── Colour‑split matrix (row's score%  asWhite / asBlack) ──");
        let mut header = format!("{:>w$} │", "", w = name_w);
        for e in &engines {
            header.push_str(&format!("{:^w$}│", short_name(e.name(), cell_w), w = cell_w));
        }
        clog!("{}", header);
        let mut sep = format!("{:─>w$}─┼", "", w = name_w);
        for _ in &engines {
            sep.push_str(&format!("{:─>w$}┼", "", w = cell_w));
        }
        clog!("{}", sep);
        for row in &engines {
            let mut line = format!("{:>w$} │", short_name(row.name(), name_w), w = name_w);
            for col in &engines {
                if row == col {
                    line.push_str(&format!("{:^w$}│", "---", w = cell_w));
                    continue;
                }
                ...
                line.push_str(&format!("{:^w$}│", cell, w = cell_w));
            }
            clog!("{}", line);
        }
        clog!("");
...
struct PairingPool {
    ...
    white_games: Vec<u32>,
    /// Seeded once at tournament start so the whole draw sequence is
    /// reproducible from the seed. `StdRng` rather than a hand-rolled
    /// generator — the only property needed here is reproducibility from
    /// a seed, which a standard seedable RNG already provides.
    rng: StdRng,
}
impl PairingPool {
